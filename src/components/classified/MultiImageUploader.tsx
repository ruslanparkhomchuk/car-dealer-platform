...
import { env } from "@/env";
import { generateThumbHashFromFile } from "@/lib/thumbhashClient";
		...
		loading: () => (
			<div className="gap-3 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5">
				{[1, 2, 3, 4, 5, 6].map((i) => (
					<Skeleton
						key={i}
						className="aspect-3/2 rounded-md w-full"
					/>
				))}
			</div>
		),
...
interface MultiImageUploaderProps extends React.InputHTMLAttributes<HTMLInputElement> {
	className?: string;
}
